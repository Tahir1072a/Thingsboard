/**
 * /api/upload/background — SCADA arka plan görseli yükleme
 *
 * POST: Multipart form data ile görsel dosyası alır,
 *       /public/uploads/backgrounds/ dizinine benzersiz isimle kaydeder.
 */
...
export async function POST(request) {
  try {
    ...
    if (useR2) {
      // ── Cloudflare R2 ──
      const r2Key = `backgrounds/${uniqueName}`;
      await r2.send(
      ...
      const url = `/api/uploads/backgrounds/${uniqueName}`;
    ...
    console.error("[POST /api/upload/background]", error);
    return NextResponse.json(
